...
public final class LogConfig implements Initialization {
	...
	public void changeOfOptimisticallyUnchokedNeighbors(int unchokeNeighbours) {
		final String msg = logSuffix + " has the optimistically unchoked neighbor "
				+ Integer.toString(unchokeNeighbours);
		logger.log(Level.INFO, msg);
	}
	...
	public void receivedHave(int peerId, int pieceIdx) {
		String msg = logSuffix + " received the 'have' message from " + Integer.toString(peerId) + " for the piece "
				+ Integer.toString(pieceIdx);
		logger.log(Level.INFO, msg);
	}
	...
	public void pieceDownloaded(int peerId, int pieceIndex, int currNumberOfPieces) {
		final String msg = logSuffix + " has downloaded the piece " + Integer.toString(pieceIndex) + " from "
				+ Integer.toString(peerId) + " Now the number of pieces it has is "
				+ Integer.toString(currNumberOfPieces);
		logger.log(Level.INFO, msg);
	}
